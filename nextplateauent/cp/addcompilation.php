...
if(isset( $_POST['submit'] ) || isset( $_POST['submit_x']) ) {
	...
	//insert record
	$text = "INSERT INTO compilations (title1, title2, description, released, audio, video, iTunes, Amazon, Beatport, eMusic, Spotify, Rhapsody, Deezer, Simfy, GooglePlay, iHeartRadio, MuveMusic, Pandora, TouchTunes, Myxer $insertTXT) VALUES ('$t1', '$t2', '$description', '$released', '$audio', '$video', '$itunes', '$amazon', '$beatport', '$emusic', '$spotify', '$rhapsody', '$deezer', '$simfy', '$google', '$iheart', '$muve', '$pandora', '$touch', '$myxer' $valuesTXT)";
	$query = mysqli_query($dbc, $text)
		or die("Error executing query: " . mysqli_error($dbc));
	//get compilation ID that was just added
	$text = "SELECT c_id FROM compilations ORDER BY c_id DESC LIMIT 1";
	$query = mysqli_query($dbc, $text)
		or die("Error executing query: " . mysqli_error($dbc));
	...
	//record in log
	$u_id = $_COOKIE['u_id'];
	$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'compilations', $c_id, 'new compilation added', (NOW() + INTERVAL 3 HOUR))";
...
<div style="width:974px; height:900px; margin:0px auto; padding-top:10px">
	...
	<form action="addcompilation.php" method="post" enctype="multipart/form-data">
    ...
    <div style="width:150px; float:left; height:35px; line-height:35px">Description: </div><div style="width:800px; height:35px; float:right"><input name="description" type="text" style="width:500px; height:25px"></div>
    ...
    <div style="width:150px; float:left; height:35px; line-height:35px">Cover: </div><div style="width:800px; height:35px; float:right"><input name="cover" type="file" style="width:250px; height:25px"><font size="-2" color="#D71F30">*Cover image is 308 pixels wide and 308 pixels high</font></div>
