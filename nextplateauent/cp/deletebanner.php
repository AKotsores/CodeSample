...
if($process) {
	...
	//query database
	$text = "SELECT * FROM banners WHERE b_id = '$id'";
	...
	if($process) {
		$text = "DELETE FROM banners WHERE b_id = '$id'";
		$query = mysqli_query($dbc, $text)
			or die("Error deleting record: " . mysqli_error($dbc));
		//record in log
		$u_id = $_COOKIE['u_id'];
		$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'banners', $id, 'record deleted', (NOW() + INTERVAL 3 HOUR))";
		mysqli_query($dbc, $text)
			or die('Error updating log: ' . mysqli_error($dbc));
	}
	mysqli_close($dbc);
}
//redirect back to banners
