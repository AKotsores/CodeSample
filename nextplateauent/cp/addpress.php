...
if(!empty($id) && is_numeric($id)) {
	...
	if(mysqli_num_rows($query)==1) {
		...
		if(isset( $_POST['submit'] ) || isset( $_POST['submit_x']) ) {
			...
			//insert record
			$text = "INSERT INTO press (artists_a_id $insertTXT) VALUES ($id $valuesTXT)";
			mysqli_query($dbc, $text)
				or die("Error executing query: " . mysqli_error($dbc));
			//record in log
			$u_id = $_COOKIE['u_id'];
			$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'artists', $id, 'press record added', (NOW() + INTERVAL 3 HOUR))";
			mysqli_query($dbc, $text)
...
<div style="width:974px; height:400px; margin:0px auto; padding-top:10px">
	...
	<form action="addpress.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
