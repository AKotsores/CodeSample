...
if(!empty($id) && is_numeric($id)) {
	...
	if(mysqli_num_rows($query)==1) {
		...
		if(isset( $_POST['submit'] ) || isset( $_POST['submit_x']) ) {
			...
			//update record
			$text = "UPDATE press SET url='$link' $insertTXT WHERE p_id=$id";
			mysqli_query($dbc, $text)
				or die("Error executing query: " . mysqli_error($dbc));
			//record in log
			$u_id = $_COOKIE['u_id'];
			$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'press', $id, 'press record updated', (NOW() + INTERVAL 3 HOUR))";
			mysqli_query($dbc, $text)
				or die('Error updating log: ' . mysqli_error($dbc));
			//close database connection
			mysqli_close($dbc);
			//refresh page
			header('Location: profile.php?id=' . $a_id);
		}
		//get press details
		$text = "SELECT * FROM press WHERE p_id=$id";
		$query = mysqli_query($dbc, $text)
...
<div style="width:974px; height:400px; margin:0px auto; padding-top:10px">
	...
	<div style="width:974px">
        <form action="editpress.php?id=<?php echo $id; ?>&a_id=<?php echo $a_id; ?>" method="post" enctype="multipart/form-data">
