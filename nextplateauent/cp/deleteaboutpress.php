...
if($process) {
	...
	if($process) {
		...
		$text = "DELETE FROM aboutpress WHERE p_id = '$id'";
		mysqli_query($dbc, $text)
			or die("Error deleting record: " . mysqli_error($dbc));
		//record in log
		$u_id = $_COOKIE['u_id'];
		$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'NPE press', $id, 'press record deleted', (NOW() + INTERVAL 3 HOUR))";
		mysqli_query($dbc, $text)
