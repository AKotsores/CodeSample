...
if($process) {
	...
	//query database
	$text = "SELECT * FROM featured WHERE artists_a_id = '$id'";
	...
	if($process) {
		...
		//record in log
		$u_id = $_COOKIE['u_id'];
		$text = "INSERT INTO logs (user_id, npe_table, table_record, user_action, time_stamp) VALUES ($u_id, 'artists', $id, 'artist removed as featured', (NOW() + INTERVAL 3 HOUR))";
